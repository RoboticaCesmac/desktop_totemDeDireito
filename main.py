...
"""
É uma função que verifica vários estados da API local que podem ser usados para ativar gatilhos no código
"""
def verificarApi():
    global falandoViaFirebase, escutarDoFirebase, ignorarPicovoice, bloquearAtivacao, escutarDaAPI, keyword_index
    url = os.getenv('CONECTA_API_URL')
    while True:
        time.sleep(0.01)
        try: 
            if modoDeConexao == "offline" or expressoesConecta.offline == True:
                dados = requests.get(url+"lerDados/Totem").json()
                
                ReproducaoVideo.alterarCorDoOlho(dados["Totem"]["corDoOlho"])

                if ReproducaoVideo.getApresentacaoLocal() == True:
                    falandoViaFirebase = True
                    falar(["Olá! Sou o Totem, um assistente virtual desenvolvido pelo centro de inovação tecnológica do Cesmac. Vocês já ouviram falar com chati gepêtê"])
                    time.sleep(1)
                    falar(["Eu utilizo essa inteligência artificial para responder dúvidas, você pode fazer qualquer pergunta para interagir!"])
                    ReproducaoVideo.setApresentacaoLocal(False)
                    falandoViaFirebase = False

                if dados["Totem"]["ignorarAtivacao"] == True: # Verifica se é para ignorar o picovoice
                    ignorarPicovoice = True
                    response = requests.put(url+"atualizarIgnorarAtivacao/Totem", json={'ignorarAtivacao': False}, headers={'Content-Type': 'application/json'})
                    time.sleep(3)
                    ignorarPicovoice = False
                elif dados["Totem"]["textoParaEscutar"] != "": # Verifica se é para falar algo pela api
                    escutarDaAPI = dados["Totem"]["textoParaEscutar"]
                    response = requests.put(url+"atualizarTextoParaEscutar/Totem", json={'textoParaEscutar': ""}, headers={'Content-Type': 'application/json'})
                elif dados["Totem"]["textoParaFalar"] != "": # Verifica se é para falar algo pela api
                    falandoViaFirebase = True
                    setarExpressao("respondendo")
                    falar([dados["Totem"]["textoParaFalar"]])
                    falandoViaFirebase = False
                    response = requests.put(url+"atualizarTextoParaFalar/Totem", json={'textoParaFalar': ""}, headers={'Content-Type': 'application/json'})
                elif dados["Totem"]["interacao"] != "": # Verifica interações para falar ou executar
                    response = requests.get(url + "perguntas?pergunta=" + dados["Totem"]["interacao"])
                    data = response.json()
                    falandoViaFirebase = True
                    if data!=[]:
                        if data[0]["tipo"] == "resposta":
                            response = requests.get(url + "respostas?id="+str(data[0]["id"]))
                            data = response.json()
                            respostas = []
                    ...
                    response = requests.put(url+"atualizarInteracao/Totem", json={'interacao': ""}, headers={'Content-Type': 'application/json'})
                    falandoViaFirebase = False

                bloquearAtivacao = requests.get(url+"Gerenciador").json()["modoDeApresentacao"]
        except Exception as e:
            pass
...
def verificarFirebase():
    global falandoViaFirebase, escutarDoFirebase, ignorarPicovoice, bloquearAtivacao, keyword_index, modoDeConexao

    while True:
        time.sleep(0.01)
        if(expressoesConecta.offline == False):
            modoDeConexao = db.reference("ModoDeConexao").get()

            if modoDeConexao == "online":
                ...
                bloquearAtivacao = db.reference("ModoDeApresentacao").get()
                ...
                # verifica se o usuário solicitou que o robô acorde pelo sistema web
                ignorarPicovoice = db.reference(inteligencia.capitalize()).child("ignorar").get()
                time.sleep(1)
                
                keyword_index = -1

                if ignorarPicovoice == True:
                    db.reference(inteligencia.capitalize()).child("ignorar").set(False)
    ...
    if(len(textos) > 0):
        ...
        try:
            if(db.reference(inteligencia.capitalize()).child("fala").child("useElevenlabs").get() == True):
                ...
                raise Exception("Elevenlabs desativado")
        except Exception as e:
            try:
                setarExpressao("respondendo")
...
def escutarMicrofone():
    global parar, escutarDoFirebase, pyAudio, pensando, escutarDaAPI
    try:
        textoFalado = ""
        # ignora a escuta do microfone se for solicitado uma interação pelo sistema web/firebase/API
        if escutarDaAPI != "":
            textoFalado = escutarDaAPI
            escutarDaAPI = ""
        if escutarDoFirebase == True:
            textoFalado = db.reference(inteligencia.capitalize()).child("escuta").child("texto").get()
...
"""
def main():
    global modoDeConexao, falandoViaFirebase, escutarDoFirebase, expressoesConecta, keyword_index, parar, numeroRespostas, pensando

    thread = Thread(target=verificarFirebase, args=())
    thread.daemon = True  # https://stackoverflow.com/questions/11815947/cannot-kill-python-script-with-ctrl-c
    thread.start()

    thread = Thread(target=verificarApi, args=())
    thread.daemon = True  # https://stackoverflow.com/questions/11815947/cannot-kill-python-script-with-ctrl-c
    thread.start()
    ...
    while True:
        ...
        if falandoViaFirebase == False:
            ...
            if escutarDaAPI != "" or escutarDoFirebase == True or ignorarPicovoice == True: 
                keyword_index = 1
            else:
                keyword_index = -1

            if bloquearAtivacao == False:
                pass # Temporário enquanto está sem Porcupine APAGAR DEPOIS # Temporário enquanto está sem Porcupine APAGAR DEPOIS # Temporário enquanto está sem Porcupine APAGAR DEPOIS # Temporário enquanto está sem Porcupine APAGAR DEPOIS
            ...
            if keyword_index >= 0:   
                ...
                while not parar:# or time.time() - inicioDelay < 60)): # Enquanto não tiver passado no mínimo 1 minutos dentro do while ele continua aguardando uma pergunta
                    ...
                    if ("vire um psicólogo" in textoFalado or "modo psicólogo" in textoFalado or "modo psicologo" in textoFalado) and numeroRespostas==0:
                        numeroRespostas = numeroRespostas + 1
                        expressoesConecta.alterarSkin("psicologo")
                        setarExpressao("respondendo")
                        respostas = ["Ok, agora eu sou um psicólogo!"]
                    # altera a skin para a padrão
                    if ("use a roupa padrão" in textoFalado or "modo citec" in textoFalado) and numeroRespostas==0:
                        numeroRespostas = numeroRespostas + 1
                        expressoesConecta.alterarSkin("citec")
                        setarExpressao("respondendo")
                        respostas = ["Ok, agora estou com a camisa do centro de inovação tecnológica!"]
                    if ("vire um personal" in textoFalado or "modo personal trainer" in textoFalado) and numeroRespostas==0:
                        numeroRespostas = numeroRespostas + 1
                        ...
                        if textoFalado and textoFalado != "":
                            if len(textoFalado)<757: #Limite que um id pode ter no firebase (evita que tente gravar uma pergunta muito grande)
                                try:
                                    ...
                                    resposta = []
                                    try:
                                        if(modoDeConexao == "offline"): #Busca a pergunta realizada na api
                                            print(" \033[33m[API]\033[0;0m Verificando API...")
                                            url = os.getenv('CONECTA_API_URL')
                                            data = requests.get(url + "perguntas?pergunta="+textoFalado).json()
                                            if len(data) != 0:
                                                data = data[0]
                                                pergunta = data["pergunta"]
                                                if data["tipo"] == "resposta":
                                                    data = requests.get(url + "respostas?id="+str(data["id"])).json()
                                                    for respostas in data:
                                                        resposta.append(respostas["texto"])    
                                                else:
                                                    try:
                                                        data = requests.get(url + "respostas?id="+str(data["id"])).json()
                                                        exec(data[0]["codigo"])
                                        ...
                                        else:   # Busca a pergunta realizada no firebase
                                    ...
                                    if pergunta == None or resposta == [""]:
                                        setarExpressao("respondendo")
                                        falar(["Aguarde, estou pensando", "Um momento, estou processando", "Aguarde um instante enquanto penso"])
                                        setarExpressao("aguardando")
                                        pensando = True
                                        startAudioThread()
                                        ...
                                        if modoDeConexao == "online":
                                            ...
                                            data = requests.get(url+"robos?nome=Totem").json()[0]
                                            skin = data["skin"]

                                        modelo = ""
                                        if modoDeConexao == "offline":
                                            modelo = requests.get(url+"Gerenciador").json()["gpt"]
                                        else:
...
def ajustarRuidos(reajustar = False):
    try:
        ...
        try:
            ...
            print(" \033[32m[LOG]\033[0;0m Inicialização concluída...")
...
"""
def checa_internet():
    url = 'https://www.google.com.br/' # utiliza o google como base
